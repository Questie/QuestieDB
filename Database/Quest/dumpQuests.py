...
def read_expansion_data(expansion):
    path = "Database\\Quest\\" + expansion
    print("Reading {} lua quest data".format(expansion))
    #Read the entire fil "QuestData.lua-table" into a string
    with open(path + "\\QuestData.lua-table", 'r') as questdata_file:
        questdata = questdata_file.read()

    # 9 quests has &, fuck it let's just replace it with "and"
    questdata = questdata.replace("&", "and")
    questdata = questdata.replace("<", "|")
    questdata = questdata.replace(">", "|")
    expansion_data[expansion] = questdata

# The main logic function, it uses the decoded data to create HTML files.
def process_expansion(expansion):
    questdata = expansion_data[expansion]
    ...
    print("Dumping quests for " + expansion)
    path = "Database\\Quest\\" + expansion

    if not os.path.exists(path + '\\_data'):
        print("Creating quest _data folder")
        os.makedirs(path + '\\_data')
    else:
        #Delete html files in _data folder
        print("Deleting old quest html _data files")
    ...
    embed_file_string += '<SimpleHTML name="QuestDataFiles" file="Interface\\AddOns\\{}\\Database\\Quest\\{}\\QuestDataTemplates.html" virtual="true" font="GameFontNormal"/>\n'.format(addon_dir, expansion)
    ...
    for dataId in questdata:
        ...
        if entries_written == range_size or entryIndex == len(questdata):# or len(lookup_data) + len(output_data) > max_file_size:
            ...
            embed_file_string += ' name="QuestData{}-{}"'.format(str(lowest_id), str(highest_id))
            embed_file_string += ' file="Interface\\AddOns\\{}\\Database\\Quest\\{}\\_data\\{}-{}.html"'.format(addon_dir, expansion, lowest_id, highest_id)
            embed_file_string += ' virtual="true"'
            embed_file_string += ' font="GameFontNormal"'
            embed_file_string += '/>\n'

            filename_data += "QuestData{}-{},".format(str(lowest_id), str(highest_id))
        ...
        if entryIndex == len(questdata) or entryIndex % 1000 == 0:
          print("Processed {}/{} {} quests".format(entryIndex, len(questdata), expansion))

    embed_file = open(path + "\\QuestDataFiles.xml", 'w')
    embed_file.write(embed_file_string)
    embed_file.write('</Ui>')
    embed_file.close()
    filename_file = open(path + "\\QuestDataTemplates.html", 'w')
    ...
    filename_file.close()
    print("Finished dumping quests for " + expansion)
...
for expansion in expansions:
    print("Decoding {} lua quest data to python quest".format(expansion))
