...
def find_lua() -> str:
    ...
    for candidate in candidates:
        ...
        if executable:
            result = subprocess.run([executable, "-e", "io.write(_VERSION)"],
...
def read_source(root: Path, flavor: str) -> FlavorSource:
    ...
    if not re.fullmatch(r"(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)\.(?:0|[1-9][0-9]*)(?:-dev\.[0-9a-f]{7})?", version):
...
def producer_commit(root: Path) -> str:
    ...
    return commit if result.returncode == 0 and re.fullmatch(r"[0-9a-f]{40}", commit) else ZERO_COMMIT
...
def build_zip(root: Path, sources: list[FlavorSource], types: list[Path], lua: str,
              flavor: str) -> Artifact:
    ...
    print("packaged %s (%d MB zipped, %d MB raw)" %
          (archive_path.relative_to(root), size // 1048576, raw // 1048576), flush=True)
    return {"flavor": "All" if flavor == "all" else flavor, "file": filename,
            "sha256": sha256(archive_path), "bytes": size, "rawBytes": raw}


def package(root: Path, flavors: list[str]) -> None:
    """Preflight inputs before clearing output, then publish a local manifest last."""
    if not flavors or len(set(flavors)) != len(flavors) or any(flavor not in FLAVORS for flavor in flavors):
    ...
    result = subprocess.run([lua, "-e", 'local c = dofile("src/config.lua"); '
    ...
    if len(values) != 2 or any(not re.fullmatch(r"[1-9][0-9]*", value) for value in values):
        raise ValueError("src/config.lua must provide positive integer contractVersion and minSupportedContract")
    ...
    if any(source.contract_version != contract for source in sources):
        raise ValueError("TOC X-Contract-Version differs from the packaged runtime; regenerate the TOCs")
    ...
    artifacts = [build_zip(root, [source], types, lua, source.flavor) for source in sources]
    if len(sources) == len(FLAVORS):
        artifacts.append(build_zip(root, sources, types, lua, "all"))
    shutil.rmtree(root / ".out/stage")

    manifest = {"producerCommit": commit, "questieCommit": questie_commit,
                "version": version, "contractVersion": contract, "minSupportedContract": minimum_contract,
...
def main() -> int:
    ...
    except (OSError, ValueError, OverflowError, subprocess.CalledProcessError, zipfile.BadZipFile) as error:
