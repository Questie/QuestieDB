...
return function(check, questiePath)
  ...
  local zhCN, zhCorrections = inputs.load(config.paths.l10n, config.flavorByName.Wrath, "Quest", "zhCN")
  ...
  local deDE, deCorrections = inputs.load(config.paths.l10n, config.flavorByName.TBC, "Quest", "deDE")
  ...
  local items, itemCorrections = inputs.load(config.paths.l10n, config.flavorByName.TBC, "Item", "deDE")
  ...
  local localizationRoot = root .. "/Localization/lookups"
  local flavor = config.flavorByName.Wrath
  for _, locale in ipairs(config.locales) do
    local file = inputs.lookupPath(localizationRoot, flavor, inputs.types.Quest, locale)
  ...
  os.remove(override)
  local ok, err = pcall(generator.assertInputs, localizationRoot, { flavor }, { Quest = true })
  ...
  check(pcall(generator.assertInputs, localizationRoot, { flavor }, { Quest = true }),
    "complete base and static translation inputs pass preflight")
  local values = generator.extract(localizationRoot, flavor, "Quest", { [1] = true, [2] = true, [3] = true, [4] = true })
  ...
  for _, locale in ipairs(config.locales) do
    local file = inputs.lookupPath(localizationRoot, flavor, inputs.types.Item, locale)
    lib.mkdirp(file:match("^(.*)/[^/]+$"))
    lib.writeAll(file, ('local m = QuestieLoader:ImportModule("l10n")\n' ..
...
function Questie.LoadTitanQuestLookupOverrides() return {} end
  ...
  for _, control in ipairs(controls) do
    ...
    local actual = generator.extract(localizationRoot, flavor, control.datatype, admitted)
  ...
  local withoutOverrides = generator.extract(localizationRoot, flavor, "Quest", admitted)
  ...
  inputs.load(localizationRoot, flavor, "Quest", "deDE")
