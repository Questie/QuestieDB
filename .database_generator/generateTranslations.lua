  ...
  if #tConcat(tbl, splitCharacter) > max_chars then
    for _, segment in ipairs(tbl) do
      ...
      if #concat_current_segments + #segment > max_chars then
        ...
        -- print("Inserting segment", concat_current_segments, #concat_current_segments, #segment)
        ...
        -- print("Adding to segment", segment, #concat_current_segments, #segment)
        tInsert(current_segment, segment)
      end
    end
    -- Insert the last segment
    if #current_segment > 0 then
      -- print("Inserting last segment", tConcat(current_segment, splitCharacter))
      tInsert(segments, tConcat(current_segment, splitCharacter))
    end
  end
...
local function create_trie(strings, stringIndex)
  ...
  for i = 1, #strings do
    ...
    local cleanedString = string.gsub(allLowerCase and string:lower() or string, "%s", "")
...
function Compile_translations_to_html(strings, addonName, translation_func)
  ...
  local lua_file = io.open(root_folder .. "/TranslationsLookup.lua", "w")
  if lua_file then
    local dump_str = printTable(trie, "  ") --dump_trie(point_to_html(trie), 1)
    lua_file:write("-- ! File generated by generateTranslations.lua --\n")
