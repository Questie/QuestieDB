...
-- location is settable rather than derived from `$PWD`.
...
function Validators.SetOutputDir(path)
  Validators.outputDir = path
  ensureOutputDir(path)
end

ensureOutputDir(Validators.outputDir)

--- Questie's drivers run one check per process and call `os.exit(1)` the moment one fails, so a
--- failing run reports the first problem and stops. Here every flavor's fifteen checks run in
--- one process and the whole picture is wanted, so the exits became a flag the driver reads and
--- clears between checks. `validators/run.lua` still exits non-zero overall.
