...
local function writeHeader(out, flavor, fileList)
  ...
  out:write("## Author: Code: Logonz Data: Muehe/TheCrux(BreakBB)/Drejjmit/Dyaxler/Cheeq/TechnoHunter/Yttrium/Anon/Everyone else\n")
...
function generate.baseToc()
  ...
  out:write("## Notes-deDE: |cFFFFD100(Quellmodus)|r Die Datenbank, die Questie verwendet. Kein generiertes Artefakt vorhanden.\n")
  out:write("## Notes-esES: |cFFFFD100(modo fuente)|r La base de datos que utiliza Questie. No hay ningún artefacto generado.\n")
  out:write("## Notes-esMX: |cFFFFD100(modo fuente)|r La base de datos que utiliza Questie. No hay ningún artefacto generado.\n")
  out:write("## Notes-frFR: |cFFFFD100(mode source)|r La base de données utilisée par Questie. Aucun artefact généré n’est présent.\n")
  out:write("## Notes-ptBR: |cFFFFD100(modo fonte)|r O banco de dados usado pelo Questie. Não há nenhum artefato gerado.\n")
  out:write("## Notes-ruRU: |cFFFFD100(режим исходных данных)|r База данных, которую использует Questie. Сгенерированный артефакт отсутствует.\n")
  out:write("## Notes-koKR: |cFFFFD100(소스 모드)|r Questie가 사용하는 데이터베이스입니다. 생성된 아티팩트가 없습니다.\n")
  out:write("## Notes-zhCN: |cFFFFD100(源模式)|r Questie 使用的数据库。不存在已生成的构建产物。\n")
  out:write("## Notes-zhTW: |cFFFFD100(來源模式)|r Questie 使用的資料庫。目前沒有已產生的建置產物。\n")
  -- Rolling previews keep a commit in their visible version even though the release tag moves.
